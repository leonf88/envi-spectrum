; $Id: //depot/idl/releases/IDL_80/idldir/lib/gauss2dfit.pro#2 $
;
...
if keyword_set(fita)  then begin
  if n_elements(a) eq 0 then message, 'Argument A must be specified when FITA is set'
endif else begin
  if Keyword_set(tilt) then fita = make_array(1,7, value = 1) else fita = make_array(1,6, value=1)

  ; if a is not specified, estimate a
  if n_elements(a) eq 0 then begin
    ...
    if (n_elements(a) ne 6+keyword_set(tilt)) then $
      MESSAGE, "Argument 'A' must have 6 elements (or 7 with /TILT)."
  endelse
